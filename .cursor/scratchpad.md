...
- [x] M7-a (Executor slice, 2026-05-13): **PRD §18 M7 start** — `QuorumCertificate` + `hash_qc` in `crates/consensus/src/qc.rs`; `AggregateSignature` borsh in `fractal-crypto`; devnet `parent_qc_hash` chains (`genesis_parent_qc` → per-block `next_parent_qc_hash_after_commit`); `apply_synced_block` validates QC + `SyncApplyError::ParentQcHash`; test `apply_synced_block_rejects_bad_parent_qc_hash`. **Planner:** BFT-7 / real BLS / vote gossip remains open.
...
- **M7-a (2026-05-13):** QC wire shape + `parent_qc_hash` chain landed; **next M7 slices:** multi-validator set, vote gossip, real BLS aggregate verify, view-change.
...
- **Next PRD §18 milestone (chain):** **M7** (BFT-7) — M7-a QC chain done; remaining: 7 validators, BLS verify, onboarding, view-change, staking/slashing.
