...
- **Next PRD milestone after M4 closure:** **M5** (Bridge to Core MVP) — off-chain backend submits `SETTLE_BATCH`, SDK claims, E2E receipts → settlement → MetaMask tFRAC. **Remaining M4 polish (Executor backlog):** MetaMask “tFRAC balance” doc / import for devnet prefund key.
...
- **Scratchpad vs code:** Before treating the next M4\* JSON-RPC line as open work, **`rg 'register_async_method'`** on `crates/rpc/src/module.rs` plus `crates/eth-wire` / `crates/node/tests` — M4-c…M4-p were already implemented while the board still showed `[ ]`.
