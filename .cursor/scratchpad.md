...
- [ ] W6-b (Executor slice, 2026-05-12, in progress): **W6** — `cap attenuate` subcommand on `fractal-wallet-cli` to mint a strictly-narrower child token (the spec's central safety property, §4 / §12). Inputs: parent token hex, parent-issuer secret, optional narrower `--workspace` / `--max-total-spend` / `--tool-mask` / `--not-after-ms` / `--cap-id` / `--nonce`. Verifies parent sig + runs `CapabilityToken::verify_attenuation_from_parent` before printing child token. Integration test extends `crates/cli/tests/wallet_cli.rs`. **Planner:** sign-off after `cargo test -p fractal-cli` green.
...
- **Wallet W6-b (2026-05-12, Executor):** Added `cap attenuate --parent-hex <hex> --issuer-secret <hex32> [--not-after-ms <t>] [--workspace <id>] [--max-total-spend <amount>] [--tool-mask <hex|dec>] [--cap-id <hex32>] [--nonce <n>]` to `fractal-wallet-cli`. Decodes parent (`borsh::try_from_slice`), re-verifies parent ed25519 sig, asserts supplied secret matches `parent.issuer`, builds a child body that clones parent's scope/caveats and optionally narrows `not_after`, `workspace`, `tool_class_mask`, and the smallest parent `MaxTotalSpend` cap (or pushes one if absent), then runs `CapabilityToken::verify_attenuation_from_parent` **before** signing so a malformed widening fails closed. Output JSON: `{ parentCapId, childCapId, subjectPub, subjectSecret, attenuationOk: true, tokenHex, ... }`. Integration tests added in `crates/cli/tests/wallet_cli.rs`: round-trip (signature ok, `parentCapId` chained, narrowed `MaxTotalSpend`), reject-widening-time, reject-widening-spend (>parent cap), reject-wrong-issuer-secret. Also added small parsing helpers `parse_hex32 / parse_hex / parse_u64_hex_or_dec`. No new workspace deps. **Awaiting `cargo test -p fractal-cli` on user machine.**
...
- **Wallet W6-b (Executor, 2026-05-12):** Pivoted W6-b away from a thin web stub (which would mostly duplicate `cap show`) toward `cap attenuate` — the wallet spec calls capability attenuation "the central primitive" / "core safety property" (§4 / §12). Added the subcommand to `fractal-wallet-cli` with positive + 3 negative integration tests. **Planner:** please run `cargo test -p fractal-cli` and (optionally) end-to-end mint → attenuate → show via `cargo run -p fractal-cli -- ...`. If sign-off, candidate next slices: **W6-c** (web stub that visualizes a `cap show` JSON paste — pure render layer, low value but completes §29 "wallet client (web)"), or **W6-d** (provider SDK surfaces — first cut of `fractal_sdk::w6` builders for `ToolIntent` / `Quote` / `ToolReceipt` matching `fractal-wallet::market`).
