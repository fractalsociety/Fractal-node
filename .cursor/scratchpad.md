...
- [ ] M7-c (Executor slice, 2026-05-13, in progress): **PRD §7 / M7 prereq** — `FRACTAL_VALIDATOR_INDEX` env var so only the round leader produces on a given host (prereq for vote gossip, view-change, per-validator BLS). New `ValidatorSet::is_proposer_for_view(view, my_index)`; `NodeInner.validator_index`; producer_loop gate (`if !is_my_turn { continue }`); integration test `crates/node/tests/m7_c_validator_index.rs`. **Planner:** sign-off after `cargo test -p fractal-consensus -p fractal-node` green.
