...
**Discovered limitation (f64 canonical round-trip):** `Scorecard` (and any artifact with `f64` fields) does **not** always bit-survive a canonical-JSON serialize→deserialize cycle, because Rust's `format!("{}", f)` (used by `canonical_json`) and `serde_json`'s float formatting can diverge such that a serde parse shifts a value (e.g. `-0.00018429404999999998` → `-0.00018429405`). This affects `Hash::of(scorecard)` equality after a round-trip — AR-06 verifies the scorecard via its **stored canonical bytes** instead. The same tension is latent in `persistence::load_proof` (which recomputes `Hash::of(scorecard)` after a serde read). **Recommended follow-up:** make `canonical_json`'s float formatting serde-consistent (or store/compare via a float-stable encoding) — but note this would change all existing content hashes, so it needs its own gated task.

**Verification run:** AR-05..09 add 18 new tests; full `fractal-society` suite = **346 passed, 0 failed**. AR-10 = 5 TS tests green (cross-language hash + signature parity on the golden package). New files are `cargo fmt`/`clippy` clean. (Pre-existing, unrelated: `crates/crypto/src/bls.rs:112` clippy lint; pre-existing `wp_*.rs` fmt drift — both outside scope.)
