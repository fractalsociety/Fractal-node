...
#[test]
fn production_verify_receipt_demo_ok() {
    let v = fractal_cli::run_argv_value(&argv(&["production", "verify-receipt"])).unwrap();
    assert_eq!(v.get("ok").and_then(Value::as_bool), Some(true));
    assert_eq!(
        v.get("toolClass").and_then(Value::as_str),
        Some("GithubWrite")
    );
}

fn policy_list_shows_builtin_templates() {
    let v = fractal_cli::run_argv_value(&argv(&["policy", "list"])).unwrap();
    let templates = v.get("templates").and_then(Value::as_array).unwrap();
    assert_eq!(templates.len(), 4, "Phase 1 trio + v2 production coding template");
    ...
    assert!(names.contains(&"tpl:coding-agent-v2-production"));
}
...
#[test]
fn cap_delegate_production_path_with_optional_e2e() {
    let parent = fractal_cli::run_argv_value(&argv(&[
        "cap",
        "mint",
        "--template",
        "2",
    ...
    .expect("coding parent mint");
    let parent_hex = parent.get("tokenHex").and_then(Value::as_str).unwrap();
    let issuer_secret = parent.get("issuerSecret").and_then(Value::as_str).unwrap();

    let delegate = fractal_cli::run_argv_value(&argv(&[
        "cap",
        "delegate",
        "--parent-hex",
        parent_hex,
    ...
    let report = delegate.get("verificationReport").unwrap();
    assert_eq!(report.get("allOk").and_then(Value::as_bool), Some(true));
    assert_eq!(
        delegate.get("sessionsDistinct").and_then(Value::as_bool),
        Some(true)
    );
    let e2e = delegate.get("e2e").unwrap();
    assert_eq!(e2e.get("settled").and_then(Value::as_bool), Some(true));

    let child_hex = delegate.get("childTokenHex").and_then(Value::as_str).unwrap();
    let show = fractal_cli::run_argv_value(&argv(&["cap", "show", child_hex])).unwrap();
    assert_eq!(show.get("signatureOk").and_then(Value::as_bool), Some(true));
    assert_eq!(show.get("budgetAccount").and_then(Value::as_u64), Some(2));
    let caveats = show.get("caveats").and_then(Value::as_array).unwrap();
    assert!(caveats
        .iter()
        .any(|c| c.as_str().unwrap_or("").contains("NoRecursion")));
}

#[test]
fn session_e2e_verifier_delegation_one_shot() {
    let v = fractal_cli::run_argv_value(&argv(&[
        "session",
        "e2e-verifier-delegation",
        "--chain-id",
        "41",
    ]))
    .expect("session e2e");
    let delegate = v.get("delegate").unwrap();
    assert_eq!(
        delegate
            .get("verificationReport")
            .and_then(|r| r.get("allOk"))
...
#[test]
fn chain_submit_create_budget_apply_local() {
    let create = fractal_cli::run_argv_value(&argv(&[
        "chain",
    ...
    assert_eq!(create.get("budgetId").and_then(Value::as_u64), Some(1));
    assert_eq!(
        create.get("totalDeposited").and_then(Value::as_str),
        Some("5000000000000000000")
    );
}

#[test]
fn chain_submit_revoke_cap_requires_registered_cap_on_apply_local() {
    let mint = fractal_cli::run_argv_value(&argv(&[
        "cap",
        "mint",
        "--template",
    ...
    .expect("mint");
    let cap_id = mint.get("capId").and_then(Value::as_str).unwrap();
    let issuer_secret = mint.get("issuerSecret").and_then(Value::as_str).unwrap();
    ...
    .unwrap_err();
    assert!(
        err.contains("WalletCapabilityNotFound"),
        "expected not-found on fresh apply-local state, got: {err}"
    );
}

#[test]
fn cap_build_and_verify_revocation_proof_round_trip() {
    let mint = fractal_cli::run_argv_value(&argv(&[
        "cap",
        "mint",
        "--template",
    ...
    let proof = fractal_cli::run_argv_value(&argv(&[
        "cap",
        "build-revocation-proof",
        "--cap-id",
        cap_id,
    ]))
    .expect("build proof");
    let root = proof.get("revocationRoot").and_then(Value::as_str).unwrap();
    let proof_hex = proof.get("proofHex").and_then(Value::as_str).unwrap();

    let verify = fractal_cli::run_argv_value(&argv(&[
        "cap",
        "verify-revocation",
        "--token-hex",
        mint.get("tokenHex").and_then(Value::as_str).unwrap(),
        "--proof-hex",
        proof_hex,
        "--revocation-root-hex",
        root,
    ...
    .expect("verify");
    assert_eq!(verify.get("revocationOk").and_then(Value::as_bool), Some(true));
}

#[test]
fn chain_submit_mint_cap_apply_local_after_offline_mint() {
    let mint = fractal_cli::run_argv_value(&argv(&[
        "cap",
        "mint",
        "--template",
    ...
    let sub = fractal_cli::run_argv_value(&argv(&[
        "chain",
        "submit-mint-cap",
        "--token-hex",
        token_hex,
    ...
    .expect("apply-local mint-cap");
    assert_eq!(sub.get("mode").and_then(Value::as_str), Some("apply-local"));
    assert_eq!(sub.get("registered").and_then(Value::as_bool), Some(true));
    assert_eq!(sub.get("capId").and_then(Value::as_str), Some(cap_id));
}

#[test]
fn reputation_snapshot_apply_local_and_preview() {
    let root = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("../..");
    let summary_path = root.join("tools/reputation-example-summary.json");
    let provider = "0x0101010101010101010101010101010101010101010101010101010101010101";

    let preview = fractal_cli::run_argv_value(&argv(&[
        "reputation",
        "preview",
        "--summary-json",
    ...
    .expect("preview");
    assert!(preview.get("scoreMilli").and_then(Value::as_str).is_some());

    let sub = fractal_cli::run_argv_value(&argv(&[
        "chain",
        "submit-reputation-snapshot",
        "--provider-id",
        provider,
        "--tool-class",
        "0",
    ...
    assert_eq!(sub.get("mode").and_then(Value::as_str), Some("apply-local"));
    assert!(sub.get("scoreMilli").and_then(Value::as_str).is_some());
}
