...
#[test]
fn create_deploys_with_nonzero_sender_nonce() {
    let deployer = addr(0xde, 0x02);
    let init_code = vec![
        0x60, 0x01, 0x60, 0x0c, 0x60, 0x00, 0x39, 0x60, 0x01, 0x60, 0x00, 0xf3, 0x00,
    ];
    ...
    let expected = create_contract_address(deployer, 1);
    let gq = genesis_parent_qc();
    ...
    assert_eq!(st.accounts.get(&deployer).unwrap().nonce, 2);
}
