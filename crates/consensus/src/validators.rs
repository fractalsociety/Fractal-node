...
//! and Phase-2 BFT-7 fixtures both derive their BLS keypairs deterministically from
...
/// Constant 32-byte prefix for BFT-7 dev fixture IKM; last byte = validator index.
...
impl ValidatorSet {
    ...
    ///
    /// Sizes: n=1→1, n=2→1, n=3→1, n=4→3, n=5→3, n=6→3, n=7→5.
    #[must_use]
    ...
    /// Ordered list (clone is cheap: ≤7 entries).
    #[must_use]
    ...
    /// fixtures; future operator-provisioned sets will return `None` once construction
    /// learns to accept pre-baked pubkeys without the dev secret.
