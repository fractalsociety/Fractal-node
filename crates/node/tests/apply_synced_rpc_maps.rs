...
fn apply_synced_block_rejects_bad_parent_qc_hash() {
    ...
    .expect("block");
    block.header.parent_qc_hash = [0u8; 32];
    assert!(matches!(
...
#[test]
fn devnet_with_bft21_fixture_has_twenty_one_validators() {
    let n = NodeInner::devnet_with_validators(fractal_consensus::ValidatorSet::phase3_bft21_fixture());
    assert_eq!(n.validators.len(), 21);
}
